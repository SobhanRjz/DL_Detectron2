...
class Pipeline:
    ...
    def _validate_config_compatibility(self) -> None:
        ...
        self.logger.info(f"✓ Configuration validated: {len(train_class_counts)} train classes, {len(test_class_counts)} test classes")
        self.logger.info(f"✓ Minimum samples per class: {min_samples_per_class} (meets requirement)")
    ...
    def print_dataset_info(self) -> None:
        ...
        self.logger.info(f"Training set: {len(train_class_counts)} classes, {len(self.data_manager.train_set)} total samples")
        self.logger.info(f"Test set: {len(test_class_counts)} classes, {len(self.data_manager.test_set)} total samples")

        # Show sample distribution
        self.logger.info("Training set class distribution:")
        for class_id in sorted(train_class_counts.keys()):
            self.logger.info(f"  Class {class_id}: {train_class_counts[class_id]} samples")

        self.logger.info("Test set class distribution:")
        for class_id in sorted(test_class_counts.keys()):
            self.logger.info(f"  Class {class_id}: {test_class_counts[class_id]} samples")

        # Show current config requirements
        min_samples = self.model_config.n_shot + self.model_config.n_query
        self.logger.info("Current config requirements:")
        self.logger.info(f"  n_way: {self.model_config.n_way}")
        self.logger.info(f"  n_shot: {self.model_config.n_shot}")
        self.logger.info(f"  n_query: {self.model_config.n_query}")
        self.logger.info(f"  Minimum samples per class needed: {min_samples}")
        ...
        self.logger.info(f"  Training set - min samples per class: {train_min} (need ≥{min_samples}) {'✓' if train_min >= min_samples else '✗'}")
        self.logger.info(f"  Test set - min samples per class: {test_min} (need ≥{min_samples}) {'✓' if test_min >= min_samples else '✗'}")
        self.logger.info(f"  Training classes: {len(train_class_counts)} (need ≥{self.model_config.n_way}) {'✓' if len(train_class_counts) >= self.model_config.n_way else '✗'}")
        self.logger.info(f"  Test classes: {len(test_class_counts)} (need ≥{self.model_config.n_way}) {'✓' if len(test_class_counts) >= self.model_config.n_way else '✗'}")
    ...
    def run(self) -> float:
        ...
        self.logger.info(f"Improvement: {final_acc - initial_acc:.2f}%")
        return final_acc
